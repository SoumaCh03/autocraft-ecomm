        ...
        >
          <div className="relative aspect-square overflow-hidden bg-dark-border rounded-xl">
            <Link to={`/product/${product._id}`}>
              {product.images?.[0] ? (
                <img src={product.images[0]} alt={product.name} className={`w-full h-full object-cover group-hover:scale-110 transition-transform duration-300 ${product.isOutOfStock ? 'opacity-50 grayscale' : ''}`} />
              ) : (
